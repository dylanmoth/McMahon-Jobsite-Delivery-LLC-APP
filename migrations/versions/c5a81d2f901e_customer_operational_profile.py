...
def upgrade() -> None:
    with op.batch_alter_table("customers") as batch:
        ...
        batch.add_column(
            sa.Column(
                "purchase_order_required", sa.Boolean(), nullable=False, server_default=sa.false()
            )
        )
        batch.add_column(
            sa.Column(
                "requires_call_ahead", sa.Boolean(), nullable=False, server_default=sa.false()
            )
        )
        batch.add_column(
            sa.Column(
        ...
        batch.add_column(
            sa.Column(
                "appointment_required", sa.Boolean(), nullable=False, server_default=sa.false()
            )
        )
        batch.add_column(
            sa.Column("forklift_available", sa.Boolean(), nullable=False, server_default=sa.false())
        )
        batch.add_column(
            sa.Column("liftgate_required", sa.Boolean(), nullable=False, server_default=sa.false())
        )
        batch.add_column(sa.Column("preferred_pickup_window", sa.String(length=120), nullable=True))
        batch.add_column(
            sa.Column("preferred_delivery_window", sa.String(length=120), nullable=True)
        )
        batch.add_column(sa.Column("receiving_hours", sa.String(length=200), nullable=True))
        batch.add_column(
            sa.Column("typical_materials", sa.Text(), nullable=False, server_default="")
        )
        batch.add_column(
            sa.Column("default_access_instructions", sa.Text(), nullable=False, server_default="")
        )
        batch.create_index(
            "ix_customers_org_primary_email", ["organization_id", "primary_email"], unique=False
        )
        batch.create_index(
            "ix_customers_org_primary_phone", ["organization_id", "primary_phone"], unique=False
        )
