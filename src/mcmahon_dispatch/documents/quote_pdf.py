...
class QuotePdfGenerator:
    ...
    def build_quote(self, destination: Path, data: QuotePdfData) -> Path:
        ...
        doc = SimpleDocTemplate(
            ...
            title=f"Quote {data.quote_number}",
            author="McMahon Jobsite Delivery LLC",
        )
        story: list[object] = []
        ...
        quote_header = [
            ...
            Paragraph(
                ...
                + (
                    f"Expires {data.expires_at.astimezone():%B %d, %Y}<br/>"
                    if data.expires_at
                    else ""
                )
        ...
        route_rows = [
            ...
            [
                ...
                Paragraph(
                    self._join_lines(
                        [data.jobsite_address, data.site_contact, data.requested_window]
                    ),
                    self.body,
                ),
            ],
        ...
        story.append(
            Paragraph(
                self._escape(data.service_description or "Jobsite delivery service"), self.body
            )
        )
        if data.materials:
            story.extend(
                [
                    Spacer(1, 4),
                    Paragraph(f"<b>Materials:</b> {self._escape(data.materials)}", self.body),
                ]
            )
        ...
        charge_rows: list[list[object]] = [
            [
                Paragraph("Description", self.body),
                Paragraph("Qty", self.right),
                Paragraph("Rate", self.right),
                Paragraph("Amount", self.right),
            ]
        ]
        ...
        charge_rows.append(
            [
                "",
                "",
                Paragraph("<b>TOTAL</b>", self.right),
                Paragraph(f"<b>{self._money(data.total_cents)}</b>", self.right),
            ]
        )
        charges = Table(
            charge_rows, colWidths=[3.85 * inch, 0.7 * inch, 1.05 * inch, 1.25 * inch], repeatRows=1
        )
        charges.setStyle(
            TableStyle(
                [
        ...
        if data.customer_notes:
            story.extend(
                [
                    Spacer(1, 12),
                    Paragraph("CUSTOMER NOTES", self.heading),
    ...
    def build_call_sheet(self, destination: Path, data: CallSheetPdfData) -> Path:
        ...
        header = Table(
            [
                [
                    self._logo(1.45 * inch, 0.68 * inch),
                    Paragraph("CONTRACTOR CALL SHEET", self.title),
                ]
            ],
        ...
        story.extend(
            [
                ...
                Spacer(1, 8),
                Paragraph(
                    f"Captured {data.created_at.astimezone():%B %d, %Y at %I:%M %p}", self.small
                ),
                Spacer(1, 8),
            ]
        )
        rows = [
            ("Company / contact", data.company_contact),
            ("Phone", data.phone),
            ("Email", data.email),
            ("Supplier / store", data.supplier_address),
            ("Jobsite", data.jobsite_address),
            ("Materials", data.materials),
            ("Dimensions", data.dimensions_text),
            ("Weight / overweight", data.weight_text),
            ("Pickup stops", data.pickup_stops),
            ("Order ready", data.order_ready),
            ("Same-day", data.same_day),
            ("PSL status", data.psl_status),
            ("Mileage", data.miles_text),
            ("Wait", data.wait_text),
            ("Trash", data.trash_text),
            ("Vehicle", data.vehicle_text),
            ("Other client scheduled", data.other_client),
            ("General notes", data.general_notes),
        ]
        table_data = [
            [
                Paragraph(f"<b>{self._escape(label)}</b>", self.body),
                Paragraph(self._escape(value or "—").replace("\n", "<br/>"), self.body),
            ]
        ...
        story.extend(
            [
                table,
                Spacer(1, 10),
                Paragraph("Internal intake document — not a customer quote.", self.small),
            ]
        )
        doc.build(story)
        return destination
    ...
    def _logo(self, width: float, height: float) -> object:
        if self.logo_path is not None and self.logo_path.is_file():
            ...
            return image
        return Paragraph(
            "<b>McMahon Dispatch</b><br/><font color='#F97316'>Jobsite Delivery</font>",
            self.heading,
        )
    ...
    def _join_lines(values: list[str]) -> str:
        return (
            "<br/>".join(QuotePdfGenerator._escape(value) for value in values if value.strip())
            or "—"
        )
