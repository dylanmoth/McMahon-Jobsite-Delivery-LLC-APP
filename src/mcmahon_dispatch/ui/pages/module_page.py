...
class ModulePage(QWidget):
    ...
    def __init__(self, key: str) -> None:
        super().__init__()
        title = QLabel(key.replace("_", " ").title()); title.setObjectName("pageTitle")
        description = QLabel(self.DESCRIPTIONS[key]); description.setObjectName("muted"); description.setWordWrap(True)
        state = QLabel("Foundation boundary established. This module has no production records yet."); state.setWordWrap(True)
        layout = QVBoxLayout(self); layout.setContentsMargins(24, 20, 24, 24); layout.addWidget(title); layout.addWidget(description); layout.addSpacing(24); layout.addWidget(state); layout.addStretch()
