...
class FirstRunAdminDialog(QDialog):
    def __init__(self, auth: AuthenticationService) -> None:
        ...
        explanation = QLabel(
            "Create the individual administrator account that will own users, pricing, backups, and audit settings. Shared production accounts are prohibited."
        )
        explanation.setWordWrap(True)
        explanation.setObjectName("muted")
        self.username = QLineEdit()
        self.username.setPlaceholderText("dmcmahon")
        self.display_name = QLineEdit()
        self.display_name.setPlaceholderText("Dylan McMahon")
    ...
    def _create(self) -> None:
        if self.password.text() != self.confirm.text():
            QMessageBox.warning(
                self, "Passwords do not match", "Enter the same password in both password fields."
            )
            return
        try:
