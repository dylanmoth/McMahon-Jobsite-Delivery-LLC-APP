...
class ReportingRepository:
    ...
    def monthly_trend(self, date_from: date, date_to: date) -> list[TrendRow]:
        ...
        expenses = dict(
            self.session.execute(
                select(
                    expense_period,
                    func.coalesce(func.sum(Expense.amount_cents + Expense.tax_cents), 0),
                )
                .where(
                    Expense.organization_id == self.organization_id,
                    Expense.deleted_at.is_(None),
        ...
        fuel = dict(
            self.session.execute(
                select(fuel_period, func.coalesce(func.sum(FuelEntry.total_cost_cents), 0))
                .where(
                    FuelEntry.organization_id == self.organization_id,
                    FuelEntry.purchased_at.between(start, end),
                )
        ...
        job_map = {
            str(p): (int(r or 0), int(pr or 0), float(m or 0), int(c or 0))
            for p, r, pr, m, c in jobs
        }
        ...
        return [
            TrendRow(
                ...
                profit_cents=job_map.get(p, (0, 0, 0, 0))[1]
                - int(expenses.get(p, 0) or 0)
                - int(fuel.get(p, 0) or 0),
    ...
    def yearly_trend(self, date_from: date, date_to: date) -> list[TrendRow]:
        ...
        return [
            TrendRow(
                p,
                job_map.get(p, (0, 0, 0, 0))[0],
                job_map.get(p, (0, 0, 0, 0))[1]
                - int(expenses.get(p, 0) or 0)
                - int(fuel.get(p, 0) or 0),
                int(fuel.get(p, 0) or 0),
                int(expenses.get(p, 0) or 0),
                job_map.get(p, (0, 0, 0, 0))[2],
                job_map.get(p, (0, 0, 0, 0))[3],
            )
            for p in periods
        ]
    ...
    def by_customer(self, date_from: date, date_to: date) -> list[RankedRow]:
        ...
        rows = self.session.execute(
            ...
            .order_by(func.sum(Job.actual_profit_cents).desc())
        ).all()
        return [
            RankedRow(str(i), n, int(r or 0), int(c or 0), int(p or 0), float(m or 0), int(j or 0))
            for i, n, r, c, p, m, j in rows
        ]
    ...
    def by_driver(self, date_from: date, date_to: date) -> list[RankedRow]:
        ...
        rows = self.session.execute(
            ...
            .join(JobAssignment, JobAssignment.driver_id == Driver.id)
    ...
    def expense_breakdown(self, date_from: date, date_to: date) -> list[ExpenseRow]:
        rows = self.session.execute(
            ...
            .order_by(func.sum(Expense.amount_cents + Expense.tax_cents).desc())
        ).all()
