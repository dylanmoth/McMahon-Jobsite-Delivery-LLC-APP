...
class InvoicePdfWriter:
    def __init__(self, logo_path: Path) -> None:
        ...
        self.small = ParagraphStyle(
            "Small", parent=self.body, fontSize=8, leading=11, textColor=SLATE
        )
        ...
        self.total = ParagraphStyle(
            "Total", parent=self.right, fontName="Helvetica-Bold", fontSize=12, textColor=DARK
        )

    def write_invoice(
        self, path: Path, organization: Organization, customer: Customer, invoice: Invoice
    ) -> None:
        doc = SimpleDocTemplate(
            ...
            author=organization.legal_name,
        )
        story = []
        ...
        meta = [
            [
                Paragraph("<b>Bill To</b>", self.body),
                Paragraph("<b>Invoice Details</b>", self.body),
            ],
            [
        ...
        charge_table = Table(
            rows, colWidths=[4.35 * inch, 0.65 * inch, 0.9 * inch, 1.0 * inch], repeatRows=1
        )
        ...
        totals = [
            ...
            [
                ...
                Paragraph(f"<b>{money(invoice.total_cents)}</b>", self.right),
            ],
            ["Payments", f"-{money(invoice.paid_cents)}"],
            [
                Paragraph("<b>Balance Due</b>", self.total),
                Paragraph(f"<b>{money(invoice.balance_cents)}</b>", self.total),
            ],
        ]
    ...
    def write_statement(
        ...
        doc = SimpleDocTemplate(
            ...
            topMargin=0.45 * inch,
            bottomMargin=0.45 * inch,
            title=f"Statement - {customer.company_name}",
            author=organization.legal_name,
        )
        story = self._header(
            organization, "CUSTOMER STATEMENT", f"{date_from:%b %d, %Y} - {date_to:%b %d, %Y}"
        )
        story.extend(
            [
        ...
        for invoice in invoices:
            ...
            transactions.append(
                (tx_date, invoice.invoice_number, "Invoice", invoice.total_cents, 0)
            )
        for payment in payments:
            transactions.append(
                (
                    ...
                    f"Payment - {payment.payment_method.replace('_', ' ').title()}",
                    0,
        ...
        for tx_date, reference, activity, charge, payment in transactions:
            rows.append(
                [
                    ...
                    reference,
                    activity,
                    money(charge) if charge else "",
                    money(payment) if payment else "",
                ]
            )
        table = Table(
            rows,
            colWidths=[1.0 * inch, 1.35 * inch, 2.65 * inch, 0.95 * inch, 0.95 * inch],
            repeatRows=1,
        )
        table.setStyle(
            TableStyle(
        ...
        balance_table = Table(
            [
                [
                    Paragraph("<b>Closing Balance</b>", self.total),
                    Paragraph(f"<b>{money(closing_balance_cents)}</b>", self.total),
                ]
            ],
            colWidths=[1.55 * inch, 1.15 * inch],
            hAlign="RIGHT",
        )
        balance_table.setStyle(
            TableStyle(
                [("LINEABOVE", (0, 0), (-1, 0), 1.2, ORANGE), ("TOPPADDING", (0, 0), (-1, -1), 7)]
            )
        )
        ...
        story.extend(
            [
                Spacer(1, 16),
                Paragraph(
                    "Please contact us if your records differ from this statement.", self.small
                ),
            ]
        )
        doc.build(story, onFirstPage=self._footer, onLaterPages=self._footer)
    ...
    def _header(self, organization: Organization, title: str, reference: str) -> list:
        ...
        if self.logo_path.exists():
            logo = Image(
                str(self.logo_path), width=1.55 * inch, height=0.74 * inch, kind="proportional"
            )
        company = Paragraph(
        ...
        table = Table(
            [[logo or company, company if logo else "", title_block]],
            colWidths=[1.65 * inch, 3.25 * inch, 2.0 * inch],
        )
        table.setStyle(
    ...
    def _footer(canvas, doc) -> None:
        ...
        canvas.drawString(
            0.55 * inch, 0.22 * inch, "McMahon Dispatch - Confidential business document"
        )
