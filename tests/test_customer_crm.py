    ...
    with database.session_factory() as session:
        organization_id = session.execute(
            __import__("sqlalchemy").select(
                __import__(
                    "mcmahon_dispatch.database.models", fromlist=["Organization"]
                ).Organization.id
            )
        ).scalar_one()
        user_id = session.execute(
            __import__("sqlalchemy").select(
                __import__("mcmahon_dispatch.database.models", fromlist=["User"]).User.id
            )
        ).scalar_one_or_none()
...
def test_customer_crm_notes(database) -> None:
    ...
    customer_id = service.save(
        CustomerSaveRequest("Apex Electric", None, "lead", 15, "Check", None, None, None, "")
    )
